...
class ParseProc:
    ...
    def getNews(self, i):
        ...
        anonses.extend(tree.find_all('div', {'class': 'news_item'}))
        pageContent.close()
        return anonses
    ...
    def selectionNews(self, anonses):
        ...
        for anons in anonses:
            try:
                dateStr = anons.find('div', {'class', 'news_date'}).get_text().strip().lower()
                date = datetime.strptime(dateStr, '%d.%m.%Y')

                header0 = anons.find('div', {'class': 'news_title'}).find('a').get_text().strip()
                ...
                if (self.startDate >= date >= self.endDate) and lenNews == 0:
                    href = anons.find('div', {'class': 'news_title'}).find('a').get('href')
        ...
        for ref in newsDict:
            ...
            try:
                try:
                    ...
                    newsPage = self.new_session().get(self.url + ref, timeout=90)
                newsTree = BeautifulSoup(newsPage.content, features='lxml').find('article', {'class', 'node node-news clearfix'}).find('div', {'class', 'content'})
                print("========================")
                ...
                try:
                    newsTree.find('div', {'class', 'second_title'}).decompose()
                except:
                    pass

                try:
                    newsTree.find('div', {'class', 'news_date'}).decompose()
                except:
                    pass

                try:
                    newsTree.find('span', {'class', 'print-link'}).decompose()
                except:
                    pass
                # парсим текст

                text = ''
                for child in newsTree.find('div', {'class', 'field field-name-field-news-body field-type-text-with-summary field-label-hidden clearfix'}).children:
                    if str(child).strip() != "":
                        try:
    ...
    def loadErrorNews(self):
        ...
        for index, row in errorNews.iterrows():
            ...
            if ln == 1000:
                ...
                if not os.path.exists(self.curDir + str(self.dirCounter) + '\\images'):
                    os.mkdir(self.curDir + str(self.dirCounter) + '\\images')
            publicDate = errorNews.loc[index]['date'].strftime('%d.%m.%Y %H:%M')
            try:
                try:
                    newsPage = self.new_session().get(self.url + errorNews.loc[index]['href'], timeout=90)
                except:
                    newsPage = self.new_session().get(self.url + errorNews.loc[index]['href'], timeout=90)
                newsTree = BeautifulSoup(newsPage.content, features='lxml').find('article', {'class',
                                                                                             'node node-news clearfix'}).find(
                    'div', {'class', 'content'})
                print("========================")
                print("Обрабатывается: " + row['href'])

                header = errorNews.loc[index]['header'][0] + errorNews.loc[index]['header'][1:].lower()
                if newsTree.find('iframe'):
                    newsTree.find('iframe').decompose()
                ...
                text = ''
                for child in newsTree.find('div', {'class',
                                                   'field field-name-field-news-body field-type-text-with-summary field-label-hidden clearfix'}).children:
                    if str(child).strip() != "":
                        try:
                            if child.get_text().strip() != "":
                                text += str(child).strip()
                ...
                self.error.loc[len(self.error)] = [errorNews.loc[index]['href'], errorNews.loc[index]['date']]
                print("новость %s вернула ошибку" % errorNews.loc[index]['href'])
