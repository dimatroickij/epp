...
class ParseProc:
    ...
    def __init__(self, totalPages, endPages, url, urlToNews, nameFolder, numberProc, codeProc, num, dirCounter):
        ...
        self.resDir = 'D:\\parseProc\\proc\\%s' % nameFolder
        self.picDir = 'D:\\parseProc\\proc\\%s\\images' % nameFolder
        ...
        self.error = pd.DataFrame(columns=['href', 'data', 'img', 'tag'])
    ...
    def getNews(self, i):
        ...
        anonses.extend(
            tree.find('div', {'class': 'news-w-items'}).find_all('div', {'class': 'col-md-4'}))
        pageContent.close()
        return anonses
    ...
    def selectionNews(self, anonses):
        ...
        for anons in anonses:
            try:
                dateStr = anons.find('div', {'class', 'n-item-date'}).get_text().strip().lower()
                monName = re.sub(r'\d+', '', dateStr).strip()
                dateStr = dateStr.replace(' ' + monName + ' ', '.' + self.monDict[monName] + '.')
                date = datetime.strptime(dateStr, '%d.%m.%Y')

                header0 = anons.find('a', {'class', 'n-item-text'}).get_text().strip()
                ...
                if (self.startDate >= date >= self.endDate) and lenNews == 0:
                    ...
                    try:
                        imgContent = anons.find('img')
                    except:
                        imgContent = ''

                    try:
                        tag = anons.find('a', {'class', 'n-item-link'}).get.text().strip()
                    except:
                        tag = ''
                    newsDict[href] = {'dateNews': dateNews, 'header': header0, 'img': imgContent, 'tag': tag}
            except:
                print("ERROR in " + href + " ")
        print('###########')
    ...
    def iterator(self, newsDict):
        ...
        for ref in newsDict:
            ...
            try:
                ...
                if newsDict[ref]['tag'] != '':
                    if newsDict[ref]['tag'].lower() == 'ветераны':
                        tag = '#' + newsDict[ref]['tag']
                    elif newsDict[ref]['tag'].lower() == 'коррупция':
                ...
                if (len(header) <= 400):
                    # лист для аппенда
                    newRow = [str(uid), header, text, str(publicDate), source, feed, '', tag, newsPics]
                ...
                self.error.loc[len(self.error)] = [ref, newsDict[ref]['dateNews'],
                                                   newsDict[ref]['img'], newsDict[ref]['tag']]
