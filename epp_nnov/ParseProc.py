...
class ParseProc:
    ...
    def getNews(self, i):
        ...
        anonses.extend(
            tree.find('div', {'id': 'news-list'}).find_all('div', {'class': 'news-item'}))
        pageContent.close()
        return anonses
    ...
    def selectionNews(self, anonses):
        ...
        for anons in anonses:
            try:
                dateStr = anons.find('p', {'class', 'news-date'}).get_text().replace('г.', '').strip()
    ...
    def iterator(self, newsDict):
        ...
        for ref in newsDict:
            ...
            try:
                ...
                header = newsTree.find('div', {'id': 'left'}).find('h1').get_text().replace('\n', '').strip()
                # print(header)

                # парсим текст
                content = newsTree.find('div', {'id': 'left'}).find('div', {'class', 'workplceNewsBlockRightText'})

                if newsTree.find('div', {'id': 'left'}).find('iframe'):
                    newsTree.find('div', {'id': 'left'}).find('iframe').decompose()
                    self.frame.loc[len(self.frame)] = [self.url + ref, header, 'iframe']
                if newsTree.find('div', {'id': 'left'}).find('audio'):
                    newsTree.find('div', {'id': 'left'}).find('audio').decompose()
                    self.frame.loc[len(self.frame)] = [self.url + ref, header, 'audio']
                if newsTree.find('div', {'id': 'left'}).find('video'):
                    newsTree.find('div', {'id': 'left'}).find('video').decompose()
