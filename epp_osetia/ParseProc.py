...
class ParseProc:
    ...
    def getNews(self, i):
        anonses = list()
        body = []
        pageContent = self.new_session().get(self.url + self.urlToNews + str((i - 1) * 20), timeout=90)
        tree = BeautifulSoup(pageContent.content, features='lxml')
        anonses.extend(tree.find('div', {'class': 'news_list'}).find_all('div', {'class', 'news_item'}))
        pageContent.close()
        return anonses
    ...
    def selectionNews(self, anonses):
        ...
        for anons in anonses:
            try:
                dateStr = anons.find('div', {'class', 'date'}).get_text().replace('/', '.').replace(' ', '').strip()

                date = datetime.strptime(dateStr, '%d.%m.%Y')

                header0 = anons.find('div', {'class', 'title'}).find('a').get_text().strip()
                ...
                if (self.startDate >= date >= self.endDate) and lenNews == 0:
                    href = anons.find('div', {'class', 'title'}).find('a').get('href')
        ...
        for ref in newsDict:
            ...
            try:
                try:
                    ...
                    newsPage = self.new_session().get(self.url + ref, timeout=90)
                newsTree = BeautifulSoup(newsPage.content, features='lxml').find('div', {'id': 'ccol'})
                print("========================")
                print("Обрабатывается: " + ref)



                header = newsTree.find('h1').get_text().replace('\n', '').strip()
                ...
                # парсим текст
                newsTree.find('h1').decompose()
                newsTree.find('div', {'class', 'breadcrumb'}).decompose()
                newsTree.find('p', {'class', 'date'}).decompose()
