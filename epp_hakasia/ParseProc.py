...
class ParseProc:
    ...
    def getNews(self, i):
        ...
        anonses.extend(tree.find('div', {'class': 'main-content'}).find_all('div', {'class', 'b-news-single'}))
        pageContent.close()
        return anonses
    ...
    def selectionNews(self, anonses):
        ...
        for anons in anonses:
            try:
                dateStr = anons.find('div', {'class', 'single-date'}).get_text().strip().lower()
        ...
        for ref in newsDict:
            ...
            try:
                try:
                    ...
                    newsPage = self.new_session().get(self.url + ref, timeout=90)
                newsTree = BeautifulSoup(newsPage.content, features='lxml').find('div', {'class': 'component news-single'})
                print("========================")
                print("Обрабатывается: " + ref)



                header = newsTree.find('h2').get_text().replace('\n', '').strip()
                ...
                # парсим текст
                content = newsTree.find('div', {'class', 'single-text'})
                ...
                imgContent2 = []
                try:
                    imgContent.extend(newsTree.find('div', {'class': 'news-single-nav-slider'}).find_all('div', {'class', 'slide-img'}))
                except:
                    pass
                newsPics = ''
                try:
                    imgContent2.extend(newsTree.find_all('img'))
                except:
                    pass

                if len(imgContent) != 0:
                    print('КАРТИНКИ!!!!!!!!!!!!!!!!!!')
                    for pic in imgContent:
                        link = pic.get('style').replace('background-image: url(', '').replace(');', '')
                        try:
                ...
                if len(imgContent2) != 0:
                    print('КАРТИНКИ!!!!!!!!!!!!!!!!!!')
                    for pic in imgContent2:
                        try:
                            link = pic.get('src')
                            if link.startswith('http') or link.startswith('https'):
                                tmp = link
