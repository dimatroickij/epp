...
class ParseProc:
    ...
    def getNews(self, i):
        ...
        anonses.extend(
            tree.find('div', {'class': 'view-content'}).find_all('div', {'class': 'views-row'}))
        pageContent.close()
        return anonses
    ...
    def selectionNews(self, anonses):
        ...
        for anons in anonses:
            try:
                dateStr = anons.find('div', {'class', 'field-name-post-date'}).get_text().strip()
                monName = re.sub(r'\d+', '', dateStr).strip()
                dateStr = dateStr.replace(' ' + monName + ' ', '.' + self.monDict[monName] + '.')
                date = datetime.strptime(dateStr, '%d.%m.%Y')

                header0 = anons.find('div', {'class', 'field-name-title'}).get_text().strip()
                ...
                if (self.startDate >= date >= self.endDate) and lenNews == 0:
                    ...
                    dateNews = date + timedelta(hours=14)
    ...
    def iterator(self, newsDict):
        ...
        for ref in newsDict:
            ...
            try:
                ...
                # парсим текст
                content = newsTree.find('div', {'class': 'field-name-body'})

                if newsTree.find('div', {'class', 'field-item even'}).find('iframe'):
                    newsTree.find('div', {'class', 'field-item even'}).find('iframe').decompose()
                    self.frame.loc[len(self.frame)] = [self.url + ref, header, 'iframe']
                if newsTree.find('div', {'class', 'field-item even'}).find('audio'):
                    newsTree.find('div', {'class', 'field-item even'}).find('audio').decompose()
                    self.frame.loc[len(self.frame)] = [self.url + ref, header, 'audio']
                if newsTree.find('div', {'class', 'field-item even'}).find('video'):
                    newsTree.find('div', {'class', 'field-item even'}).find('video').decompose()
                    self.frame.loc[len(self.frame)] = [self.url + ref, header, 'video']

                text = ''
                for child in content.find('div', {'class', 'field-item even'}).children:
                    if str(child).strip() != "":
                        try:
                ...
                # pics
                pics = newsTree.find('div', {'class', 'content'}).find_all('img')
                ...
                try:
                    sourceText = newsTree.find('div', {'class', 'field-name-field-section'}).find('a').get_text().strip()
                except:
                    sourceText = ''
