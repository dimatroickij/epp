...
class ParseProc:
    ...
    def selectionNews(self, anonses):
        ...
        for anons in anonses:
            try:
                dateStr = anons.find('div', {'class': 'views-field-field-data-news'}).get_text().strip().lower().strip()
        ...
        for ref in newsDict:
            ...
            try:
                try:
                    ...
                    newsPage = self.new_session().get(self.url + ref, timeout=90)
                newsTree = BeautifulSoup(newsPage.content, features='lxml').find('div', {'class', 'view-content'}).find('div', {'class', 'views-row'})
                print("========================")
                    ...
                    for pic in pics:
                        ...
                        try:
                            if link.startswith('http') or link.startswith('https'):
                                ...
                                link = link.replace(' ', '')
                            elif link.find('prokuror.hbr.ru') != -1:
                                link.replace('prokuror.hbr.ru', '')
                            else:
                                tmp = self.url + link
